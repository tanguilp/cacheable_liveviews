defmodule CacheableLiveviewsWeb.AdminAuth do
  use CacheableLiveviewsWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias CacheableLiveviews.Backoffice

  # Make the remember me cookie valid for 60 days.
  # If you want bump or reduce this value, also change
  # the token expiry itself in AdminToken.
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_cacheable_liveviews_web_admin_remember_me"
  ...
  def log_out_admin(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      CacheableLiveviewsWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_admin:
      ...
      defmodule CacheableLiveviewsWeb.PageLive do
        use CacheableLiveviewsWeb, :live_view

        on_mount {CacheableLiveviewsWeb.AdminAuth, :mount_current_admin}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{CacheableLiveviewsWeb.AdminAuth, :ensure_authenticated}] do
